...
lives in Git and in its Appendices A to D, and Appendix E records how the build diverged from
...
`source busy — poller cycle in progress` plus the state summary and exits 0. A busy system is not a failure. Its requests go through the same request gate and
...
- `sources` is the poll schedule per event source: all five of §9's components, plus
  which one is binding.
- `ledger` is the core hourly ledger: window status, `poll` used/allowance,
  `detail_fallback` used/allowance (renamed from `enrichment`, since it budgets the
  bounded detail lane, default 40/hour), and the `actor_requests`/`repository_requests`
  share pairs.
- `search_ledger` is the per-minute Search ledger: `present`, the observed
...
- `scheduler` is every staged tunable, published so a reading needs no shell access:
  ...
  metrics windows.
- `enrichment` is per entity class: the four `enrichment_status` counts,
...
- `batches` is batch quality over the metrics window, for all four request-kind ×
...
- `throughput` is measured rates over the same window: arrivals, completions,
...
- `coverage` is §11's three coverage percentages, unchanged.
...
The list omits `raw_payload`, a multi-kilobyte TOASTed `jsonb` column nobody
...
| `enrichment_observations` | one raw observed item, append-only and read-only after persist | `id`; fingerprint + `observed_at` describe content | `Ingestion::PageWriter` (event source), `Enrichment::ObservationRecorder` (search/detail) |
...
`push_events` is the durability boundary.
...
| `github_push_id` | `bigint` | `payload.push_id`; indexed but not unique, since GitHub does not guarantee it is |
...
`event_sources` has five independent scheduling columns, never one collapsed timestamp:
...
`quarantined_events` uses `payload_fingerprint` as its sole identity: SHA-256 of compact
...
`github_api_budget` is a `CHECK (id = 1)` singleton, because the unauthenticated quota
...
- `batch_pending` is durable backlog, claimed FIFO (`created_at, id`) into Search
  batches of up to `SEARCH_BATCH_SIZE`.
- `batch_in_flight` and `detail_in_flight` are leased to one request attempt; a crashed
  worker's lease expires by arithmetic and the rows are reclaimed
  (`enrichment.stale_lease_reclaimed`).
- `detail_pending` is admitted to the fallback lane because the batch came back
...
- `contract_complete` is the useful-data contract evaluated against a durably
  observed, ID-validated item. Nullable fields count as evaluated, not missing.
- `terminal` is an entity-specific fact only: a `404`/`410` immediately, or a detail
...
At limit 60, reserve 8, configured detail allowance 40, cadence 300s, and one source, the
...
spends the same sixty requests an hour. The ledger cannot prevent that, since it coordinates this
...
| `ENRICHMENT_CYCLE_BUDGET_SECONDS` | `55` | One enrichment cycle's time budget, below the 60-second dispatch tick and above the pacing interval |
...
behind the enrichment workload. Multi-queue workers are declared as a YAML list, since Solid
...
- `INSERT … ON CONFLICT (github_event_id) DO NOTHING RETURNING id` is the deduplication gate.
...
|---|---|---|
| No `push_events` after 10 minutes in live mode | Documented 30s to 6h feed latency plus the 5-minute cadence, not a fault | `curl -s localhost:3000/status \| jq .sources` |
...
1. Catch-up is measured, never guaranteed. One observed live page held ~92 to 95
