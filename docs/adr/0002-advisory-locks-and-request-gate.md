...
"GPI" — rather than a hash of a seed string, so `SELECT * FROM pg_locks WHERE locktype =
'advisory'` is readable during an incident without running Ruby to decode it, and so a
...
- Under RSpec's transactional fixtures the gate's `SET LOCAL` runs inside a savepoint and
  ...
  Harmless — it only bounds lock waits — but noted so nobody "fixes" it.
