...
observed live page of `/events` held ~92 to 95 push events referencing ~89 distinct actors and
...
3. The flag rides on `Github::Request` rather than on a `RequestExecutor` argument.
   ...
   authorization survives both. Re-reserving a borrowed request under the guarantee
...
- The borrow fact is stale by construction. A poll can persist a new candidate between
  the eligibility query and the debit. The exposure is bounded to one request, since the runner
...
- `guarantee + other_unused` as the borrow cap. Rejected as equivalent but with more
  moving parts: it needs a `max(…, 0)`, a read of the other class's counter, and a guard
  that cannot be read off the statement.
