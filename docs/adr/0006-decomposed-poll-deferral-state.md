...
1. `--force` cannot tell which part it may bypass. §9 lets the one-shot ignore the
   ...
   everything, which violates the floor and risks the budget, or bypasses nothing, and the
   demo does not work.
...
`--force` is demonstrable and safe at the same time, and the two
...
- A pre-flight deferral writes no run row, so `ingestion_runs` is a history of *attempts*
  ...
  deliberate, since under PR 8's recurring task the alternative is dozens of zero-count rows
  an hour diluting every counter in the table, but it is a real limitation.
