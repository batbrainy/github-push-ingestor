...
| 1.13 to 1.17 | three suite runs (`run --rm --build test`, `run --rm test`, fixed seed 4242) with dev-DB observations before/after | 1,734 + 10 examples, 0 failures, three consecutive times; dev `push_events`, `solid_queue_jobs`, and `setup` container finish time unchanged | 87s | pass |
