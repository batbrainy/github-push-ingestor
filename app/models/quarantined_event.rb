class QuarantinedEvent < ApplicationRecord
  validates :payload_fingerprint, :raw_payload, :first_received_at,
            :last_received_at, presence: true
  ...
  #
  # last_received_at uses GREATEST rather than a plain assignment: a delayed or
  # clock-skewed replay must not move the column backwards, which would contradict
  # its stated meaning ("most recent observation"). This is a deliberate hardening of
  # the plan's snippet, not a change of intent.
